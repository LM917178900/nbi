...
#JAVA_OPT="${JAVA_OPT} -Xdebug -Xrunjdwp:transport=dt_socket,suspend=n,server=y,address=0.0.0.0:6666"
...
# 打印启动命令
STARTUP_LOG="${STARTUP_LOG}application background startup command: nohup java ${JAVA_OPT} -jar -Xbootclasspath/a:${BASE_PATH} ${BASE_PATH}/lib/${APPLICATION_JAR} --spring.config.location=${CONFIG_DIR} --logging.config=${CONFIG_DIR}logback.xml > ${LOG_PATH} 2>&1 &\n"


#======================================================================
# 执行启动命令：后台启动项目,并将日志输出到项目根目录下的logs文件夹下
#======================================================================
nohup java ${JAVA_OPT} -jar -Xbootclasspath/a:${BASE_PATH} ${BASE_PATH}/lib/${APPLICATION_JAR} -Des.set.netty.runtime.available.processors=false -Dspring.config.location=${CONFIG_DIR} -Dlogging.config=${CONFIG_DIR}logback.xml > ${LOG_PATH} 2>&1 &
